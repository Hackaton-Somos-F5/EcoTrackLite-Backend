...
| :--- | :--- | :--- |
| **TK-004-01** | Coordinar con Persona A para modificar la respuesta de `POST /residuos` | Hecho |
| **TK-004-02** | Llamar a `calcular_alertas(db)` después del `db.commit()` en el POST | Hecho |
| **TK-004-03** | Actualizar `ResiduoResponse` para incluir campo `alertas` opcional | Hecho |
| **TK-004-04** | Verificar que los tests de Persona A siguen pasando tras el cambio | Hecho |
